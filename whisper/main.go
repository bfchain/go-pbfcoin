...
func main() {
	...
	// Create an pbfcoin peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting pbfcoin peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start pbfcoin peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that sompbfing works
