...
import (
	...
	"github.com/pbfcoin/go-pbfcoin/crypto/sha3"
...
func TestHeaderStorage(t *testing.T) {
	...
	if entry := Gpbfeader(db, header.Hash()); entry != nil {
	...
	if entry := GpbfeaderRLP(db, header.Hash()); entry == nil {
...
func TestBlockStorage(t *testing.T) {
	...
	if entry := Gpbfeader(db, block.Hash()); entry != nil {
		t.Fatalf("Non existent header returned: %v", entry)
	}
...
func TestHeadStorage(t *testing.T) {
	...
	if entry := GpbfeadHeaderHash(db); entry != (common.Hash{}) {
		t.Fatalf("Non head header entry returned: %v", entry)
	}
	if entry := GpbfeadBlockHash(db); entry != (common.Hash{}) {
		t.Fatalf("Non head block entry returned: %v", entry)
	}
	if entry := GpbfeadFastBlockHash(db); entry != (common.Hash{}) {
	...
	if entry := GpbfeadHeaderHash(db); entry != blockHead.Hash() {
		t.Fatalf("Head header hash mismatch: have %v, want %v", entry, blockHead.Hash())
	}
	if entry := GpbfeadBlockHash(db); entry != blockFull.Hash() {
		t.Fatalf("Head block hash mismatch: have %v, want %v", entry, blockFull.Hash())
	}
	if entry := GpbfeadFastBlockHash(db); entry != blockFast.Hash() {
...
func TestMipmapChain(t *testing.T) {
	...
	var (
		db, _   = pbfdb.NewLDBDatabase(dir, 16)
