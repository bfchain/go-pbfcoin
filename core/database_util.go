...
func GetCanonicalHash(db pbfdb.Database, number uint64) common.Hash {
...
// GpbfeadHeaderHash retrieves the hash of the current canonical head block's
// header. The difference between this and GpbfeadBlockHash is that whereas the
// last block hash is only updated upon a full block import, the last header
// hash is updated already at header import, allowing head tracking for the
// light synchronization mechanism.
func GpbfeadHeaderHash(db pbfdb.Database) common.Hash {
...
// GpbfeadBlockHash retrieves the hash of the current canonical head block.
func GpbfeadBlockHash(db pbfdb.Database) common.Hash {
...
// GpbfeadFastBlockHash retrieves the hash of the current canonical head block during
// fast synchronization. The difference between this and GpbfeadBlockHash is that
// whereas the last block hash is only updated upon a full block import, the last
// fast hash is updated when importing pre-processed blocks.
func GpbfeadFastBlockHash(db pbfdb.Database) common.Hash {
...
// GpbfeaderRLP retrieves a block header in its raw RLP database encoding, or nil
// if the header's not found.
func GpbfeaderRLP(db pbfdb.Database, hash common.Hash) rlp.RawValue {
	data, _ := db.Get(append(append(blockPrefix, hash[:]...), headerSuffix...))
	return data
}

// Gpbfeader retrieves the block header corresponding to the hash, nil if none
// found.
func Gpbfeader(db pbfdb.Database, hash common.Hash) *types.Header {
	data := GpbfeaderRLP(db, hash)
	if len(data) == 0 {
		return nil
	}
...
func GetBodyRLP(db pbfdb.Database, hash common.Hash) rlp.RawValue {
...
func GetBody(db pbfdb.Database, hash common.Hash) *types.Body {
...
// none found.
func GetTd(db pbfdb.Database, hash common.Hash) *big.Int {
...
func GetBlock(db pbfdb.Database, hash common.Hash) *types.Block {
	// Retrieve the block header and body contents
	header := Gpbfeader(db, hash)
	if header == nil {
		return nil
	}
...
func GetBlockReceipts(db pbfdb.Database, hash common.Hash) types.Receipts {
...
func GetTransaction(db pbfdb.Database, hash common.Hash) (*types.Transaction, common.Hash, uint64, uint64) {
...
func GetReceipt(db pbfdb.Database, txHash common.Hash) *types.Receipt {
...
func WriteCanonicalHash(db pbfdb.Database, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db pbfdb.Database, hash common.Hash) error {
...
func WriteHeadBlockHash(db pbfdb.Database, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db pbfdb.Database, hash common.Hash) error {
...
func WriteHeader(db pbfdb.Database, header *types.Header) error {
...
func WriteBody(db pbfdb.Database, hash common.Hash, body *types.Body) error {
...
func WriteTd(db pbfdb.Database, hash common.Hash, td *big.Int) error {
...
func WriteBlock(db pbfdb.Database, block *types.Block) error {
...
func WriteBlockReceipts(db pbfdb.Database, hash common.Hash, receipts types.Receipts) error {
...
func WriteTransactions(db pbfdb.Database, block *types.Block) error {
	batch := db.NewBatch()
...
func WriteReceipts(db pbfdb.Database, receipts types.Receipts) error {
	batch := db.NewBatch()
...
func DeleteBlockReceipts(db pbfdb.Database, hash common.Hash) {
	db.Delete(append(blockReceiptsPrefix, hash.Bytes()...))
}

// DeleteTransaction removes all transaction data associated with a hash.
func DeleteTransaction(db pbfdb.Database, hash common.Hash) {
...
func DeleteReceipt(db pbfdb.Database, hash common.Hash) {
...
// network transitions to pbf/63.
func GetBlockByHashOld(db pbfdb.Database, hash common.Hash) *types.Block {
...
// MIP bloom bin.
func WriteMipmapBloom(db pbfdb.Database, number uint64, receipts types.Receipts) error {
	batch := db.NewBatch()
...
func GetMipmapBloom(db pbfdb.Database, number, level uint64) types.Bloom {
