...
// Package core implements the pbfcoin consensus protocol.
package core
...
// available in the database. It initialiser the default pbfcoin Validator and
// Processor.
func NewBlockChain(chainDb pbfdb.Database, pow pow.PoW, mux *event.TypeMux) (*BlockChain, error) {
	...
	if bc.genesisBlock == nil {
		...
		glog.V(logger.Info).Infoln("WARNING: Wrote default pbfcoin genesis block")
	}
	...
	for hash, _ := range BadHashes {
		if header := bc.Gpbfeader(hash); header != nil {
			glog.V(logger.Error).Infof("Found bad hash, rewinding chain to block #%d [%x��因", header.Number, header.ParentHash[:4])
			bc.Spbfead(header.Number.Uint64() - 1)
...
func (self *BlockChain) loadLastState() error {
	...
	head := GpbfeadBlockHash(self.chainDb)
	if head == (common.Hash{}) {
	...
	if head := GpbfeadHeaderHash(self.chainDb); head != (common.Hash{}) {
		if header := self.Gpbfeader(head); header != nil {
			self.currentHeader = header
		}
	}
	// Restore the last known head fast block
	self.currentFastBlock = self.currentBlock
	if head := GpbfeadFastBlockHash(self.chainDb); head != (common.Hash{}) {
		if block := self.GetBlock(head); block != nil {
	...
	glog.V(logger.Info).Infof("Last header: #%d [%x��因 TD=%v", self.currentHeader.Number, self.currentHeader.Hash().Bytes()[:4], headerTd)
	glog.V(logger.Info).Infof("Last block: #%d [%x��因 TD=%v", self.currentBlock.Number(), self.currentBlock.Hash().Bytes()[:4], blockTd)
	glog.V(logger.Info).Infof("Fast block: #%d [%x��因 TD=%v", self.currentFastBlock.Number(), self.currentFastBlock.Hash().Bytes()[:4], fastTd)

	return nil
}

// Spbfead rewinds the local chain to a new head. In the case of headers, everything
// above the new head will be deleted and the new one set. In the case of blocks
// though, the head may be further rewound if block bodies are missing (non-archive
// nodes after a fast sync).
func (bc *BlockChain) Spbfead(head uint64) {
	bc.mu.Lock()
	defer bc.mu.Unlock()
	...
	for bc.currentHeader != nil && bc.currentHeader.Number.Uint64() > head {
		...
		bc.currentHeader = bc.Gpbfeader(bc.currentHeader.ParentHash)
	}
...
func (self *BlockChain) FastSyncCommitHead(hash common.Hash) error {
	...
	if block == nil {
		return fmt.Errorf("non existent block [%x��因", hash[:4])
	}
	...
	glog.V(logger.Info).Infof("committed block #%d [%x��因 as new head", block.Number(), hash[:4])
	return nil
}
...
func (bc *BlockChain) HasHeader(hash common.Hash) bool {
	return bc.Gpbfeader(hash) != nil
}

// Gpbfeader retrieves a block header from the database by hash, caching it if
// found.
func (self *BlockChain) Gpbfeader(hash common.Hash) *types.Header {
	// Short circuit if the header's already in the cache, retrieve otherwise
	if header, ok := self.headerCache.Get(hash); ok {
		return header.(*types.Header)
	}
	header := Gpbfeader(self.chainDb, hash)
	if header == nil {
		return nil
	}
...
// GpbfeaderByNumber retrieves a block header from the database by number,
// caching it (associated with its hash) if found.
func (self *BlockChain) GpbfeaderByNumber(number uint64) *types.Header {
	hash := GetCanonicalHash(self.chainDb, number)
	if hash == (common.Hash{}) {
		return nil
	}
	return self.Gpbfeader(hash)
}
...
func (self *BlockChain) GetBlockHashesFromHash(hash common.Hash, max uint64) []common.Hash {
	...
	for i := uint64(0); i < max; i++ {
		if header = self.Gpbfeader(header.ParentHash); header == nil {
			break
		}
...
func (self *BlockChain) writeHeader(header *types.Header) error {
	...
	if td.Cmp(self.GetTd(self.currentHeader.Hash())) > 0 {
		...
		head := self.Gpbfeader(header.ParentHash)
		for GetCanonicalHash(self.chainDb, head.Number.Uint64()) != head.Hash() {
			...
			head = self.Gpbfeader(head.ParentHash)
		}
...
//
// The verify parameter can be used to fine tune whpbfer nonce verification
...
func (self *BlockChain) InsertHeaderChain(chain []*types.Header, checkFreq int) (int, error) {
	...
	process := func(worker int) {
		for index := range tasks {
			...
			var err error
			if index == 0 {
				err = self.Validator().ValidateHeader(header, self.Gpbfeader(header.ParentHash), checkPow)
			} else {
	...
	glog.V(logger.Info).Infof("imported %d header(s) (%d ignored) in %v. #%v [%x��?/ %x��因", stats.processed, stats.ignored,
...
func (self *BlockChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if self.currentHeader.Hash() == hash {
			self.currentHeader = self.Gpbfeader(self.currentHeader.ParentHash)
...
func (self *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	process := func(worker int) {
		for index := range tasks {
			...
			if !self.HasHeader(block.Hash()) {
				errs[index] = fmt.Errorf("containing header #%d [%x��因 unknown", block.Number(), block.Hash().Bytes()[:4])
				atomic.AddInt32(&failed, 1)
				return
			}
	...
	glog.V(logger.Info).Infof("imported %d receipt(s) (%d ignored) in %v. #%d [%x��?/ %x��因", stats.processed, stats.ignored,
		time.Since(start), last.Number(), first.Hash().Bytes()[:4], last.Hash().Bytes()[:4])
