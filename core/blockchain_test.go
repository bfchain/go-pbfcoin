...
func thePow() pow.PoW {
	pow, _ := pbfash.NewForTesting()
	return pow
}

func theBlockChain(db pbfdb.Database, t *testing.T) *BlockChain {
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	if full {
		...
		hash1 = blockchain.GpbfeaderByNumber(uint64(i)).Hash()
		hash2 = blockchain2.GpbfeaderByNumber(uint64(i)).Hash()
	}
	if hash1 != hash2 {
...
func testHeaderChainImport(chain []*types.Header, blockchain *BlockChain) error {
	for _, header := range chain {
		...
		if err := blockchain.Validator().ValidateHeader(header, blockchain.Gpbfeader(header.ParentHash), false); err != nil {
			return err
		}
...
func TestLastBlock(t *testing.T) {
	...
	bchain.insert(block)
	if block.Hash() != GpbfeadBlockHash(db) {
...
func chm(genesis *types.Block, db pbfdb.Database) *BlockChain {
	var eventMux event.TypeMux
...
func testReorg(t *testing.T, first, second []int, td int64, full bool) {
	...
	if full {
		...
		for header := bc.GpbfeaderByNumber(bc.CurrentHeader().Number.Uint64() - 1); header.Number.Uint64() != 0; prev, header = header, bc.GpbfeaderByNumber(header.Number.Uint64()-1) {
	...
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		for j := 0; j < i-failAt; j++ {
			if full {
				...
				if header := blockchain.GpbfeaderByNumber(failNum + uint64(j)); header != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archiveDb, _ := pbfdb.NewMemDatabase()
	...
	for i := 0; i < len(blocks); i++ {
		...
		if fheader, aheader := fast.Gpbfeader(hash), archive.Gpbfeader(hash); fheader.Hash() != aheader.Hash() {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	lightDb, _ := pbfdb.NewMemDatabase()
