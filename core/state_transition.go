...
func (self *StateTransition) buyGas() error {
	...
	if sender.Balance().Cmp(mgval) < 0 {
		return fmt.Errorf("insufficient pbf for gas (%x). Req %v, has %v", sender.Address().Bytes()[:4], mgval, sender.Balance())
	}
...
func (self *StateTransition) refundGas() {
	// Return pbf for remaining gas to the sender account,
