...
func newWorker(coinbase common.Address, pbf core.Backend) *worker {
	worker := &worker{
		pbf:            pbf,
		mux:            pbf.EventMux(),
		chainDb:        pbf.ChainDb(),
		...
		chain:          pbf.BlockChain(),
		proc:           pbf.BlockChain().Validator(),
...
func (self *worker) setpbferbase(addr common.Address) {
	self.mu.Lock()
	...
	for {
		for result := range self.recv {
			...
			if self.fullValidation {
				...
				auxValidator := self.pbf.BlockChain().AuxValidator()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	state, err := state.New(parent.Root(), self.pbf.ChainDb())
	if err != nil {
		return err
	}
	work := &Work{
	...
	accounts, _ := self.pbf.AccountManager().Accounts()
...
func (self *worker) commitNewWork() {
	...
	/* //approach 1
	transactions := self.pbf.TxPool().GetTransactions()
	...
	*/
	...
	for _, tx := range self.pbf.TxPool().GetTransactions() {
	...
	*/
	...
	self.pbf.TxPool().RemoveTransactions(work.lowGasTxs)
