...
type TrieSync struct {
	database pbfdb.Database           // State database for storing all the assembled node data
	requests map[common.Hash]*request // Pending requests pertaining to a key hash
	queue    *prque.Prque             // Priority queue with the pending requests
}

// NewTrieSync creates a new trie data download scheduler.
func NewTrieSync(root common.Hash, database pbfdb.Database, callback TrieSyncLeafCallback) *TrieSync {
	ts := &TrieSync{
		database: database,
...
func (s *TrieSync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback TrieSyncLeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togpbfer
...
func (s *TrieSync) children(req *request) ([]*request, error) {
	// Gather all the children of the node, irrelevant whpbfer known or not
	type child struct {
		node  *node
		depth int
	}
...
func (s *TrieSync) commit(req *request, batch pbfdb.Batch) (err error) {
