...
import (
	...
	"github.com/pbfcoin/go-pbfcoin/pbf/fetcher"
...
// NewProtocolManager returns a new pbfcoin sub protocol manager. The pbfcoin sub protocol manages peers capable
// with the pbfcoin network.
func NewProtocolManager(fastSync bool, networkId int, mux *event.TypeMux, txpool txPool, pow pow.PoW, blockchain *core.BlockChain, chaindb pbfdb.Database) (*ProtocolManager, error) {
	// Figure out whpbfer to allow fast sync or not
	...
	manager.downloader = downloader.New(chaindb, manager.eventMux, blockchain.HasHeader, blockchain.HasBlockAndState, blockchain.Gpbfeader,
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	// Unregister the peer from the downloader and pbfcoin peer set
...
func (pm *ProtocolManager) Stop() {
	...
	glog.V(logger.Info).Infoln("Stopping pbfcoin protocol handler...")
	...
	glog.V(logger.Info).Infoln("pbfcoin protocol handler stopped")
}
...
// handle is the callback invoked to manage the life cycle of an pbf peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	glog.V(logger.Debug).Infof("%v: peer connected [%s]", p, p.Name())

	// Execute the pbfcoin handshake
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version < pbf62 && msg.Code == GetBlockHashesMsg:
	...
	case p.version < pbf62 && msg.Code == GetBlockHashesFromNumberMsg:
	...
	case p.version < pbf62 && msg.Code == BlockHashesMsg:
	...
	case p.version < pbf62 && msg.Code == GetBlocksMsg:
	...
	case p.version < pbf62 && msg.Code == BlocksMsg:
	...
	case p.version >= pbf62 && msg.Code == GetBlockHeadersMsg:
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit && len(headers) < downloader.MaxHeaderFetch {
			...
			if query.Origin.Hash != (common.Hash{}) {
				origin = pm.blockchain.Gpbfeader(query.Origin.Hash)
			} else {
				origin = pm.blockchain.GpbfeaderByNumber(query.Origin.Number)
			}
			if origin == nil {
				break
			}
			...
			case query.Origin.Hash != (common.Hash{}) && query.Reverse:
				...
				for i := 0; i < int(query.Skip)+1; i++ {
					if header := pm.blockchain.Gpbfeader(query.Origin.Hash); header != nil {
			...
			case query.Origin.Hash != (common.Hash{}) && !query.Reverse:
				...
				if header := pm.blockchain.GpbfeaderByNumber(origin.Number.Uint64() + query.Skip + 1); header != nil {
	...
	case p.version >= pbf62 && msg.Code == BlockHeadersMsg:
	...
	case p.version >= pbf62 && msg.Code == GetBlockBodiesMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= pbf62 && msg.Code == BlockBodiesMsg:
	...
	case p.version >= pbf63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= pbf63 && msg.Code == NodeDataMsg:
	...
	case p.version >= pbf63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
		...
		for bytes < softResponseLimit && len(receipts) < downloader.MaxReceiptFetch {
			...
			if results == nil {
				if header := pm.blockchain.Gpbfeader(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
					continue
				}
			}
	...
	case p.version >= pbf63 && msg.Code == ReceiptsMsg:
...
// pbfNodeInfo represents a short summary of the pbfcoin sub-protocol metadata known
// about the host peer.
type pbfNodeInfo struct {
	Network    int      `json:"network"`    // pbfcoin network ID (0=Olympic, 1=Frontier, 2=Morden)
...
func (self *ProtocolManager) NodeInfo() *pbfNodeInfo {
	return &pbfNodeInfo{
