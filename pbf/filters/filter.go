...
// Create a new filter which uses a bloom filter on blocks to figure out whpbfer a particular block
// is interesting or not.
func New(db pbfdb.Database) *Filter {
	return &Filter{db: db}
}
...
func (self *Filter) Find() vm.Logs {
	latestBlock := core.GetBlock(self.db, core.GpbfeadBlockHash(self.db))
