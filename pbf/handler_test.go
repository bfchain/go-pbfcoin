...
func testGetNodeData(t *testing.T, protocol int) {
	...
	generator := func(i int, block *core.BlockGen) {
		switch i {
		case 0:
			// In block 1, the test bank sends account #1 some pbfer.
			tx, _ := types.NewTransaction(block.TxNonce(testBankAddress), acc1Addr, big.NewInt(10000), params.TxGas, nil, nil).SignECDSA(testBankKey)
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more pbfer to account #1.
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*pbfdb.MemDatabase).Keys() {
	...
	statedb, _ := pbfdb.NewMemDatabase()
