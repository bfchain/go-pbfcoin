...
func NewGasPriceOracle(pbf *pbfcoin) *GasPriceOracle {
	minprice := pbf.GpoMinGasPrice
	if minprice == nil {
		minprice = big.NewInt(gpoDefaultMinGasPrice)
	}
	minbase := new(big.Int).Mul(minprice, big.NewInt(100))
	if pbf.GpobaseCorrectionFactor > 0 {
		minbase = minbase.Div(minbase, big.NewInt(int64(pbf.GpobaseCorrectionFactor)))
	}
...
func (gpo *GasPriceOracle) init() {
	gpo.initOnce.Do(func() {
		gpo.processPastBlocks(gpo.pbf.BlockChain())
		go gpo.listenLoop()
	})
}
...
func (self *GasPriceOracle) listenLoop() {
	events := self.pbf.EventMux().Subscribe(core.ChainEvent{}, core.ChainSplitEvent{})
...
func (self *GasPriceOracle) processBlock(block *types.Block) {
	...
	if lastBase.Cmp(lp) < 0 {
		corr = self.pbf.GpobaseStepUp
	} else {
		corr = -self.pbf.GpobaseStepDown
	}
...
func (self *GasPriceOracle) lowestPrice(block *types.Block) *big.Int {
	...
	receipts := core.GetBlockReceipts(self.pbf.ChainDb(), block.Hash())
	if len(receipts) > 0 {
	...
	if new(big.Int).Mul(gasUsed, big.NewInt(100)).Cmp(new(big.Int).Mul(block.GasLimit(),
		big.NewInt(int64(self.pbf.GpoFullBlockRatio)))) < 0 {
...
func (self *GasPriceOracle) SuggestPrice() *big.Int {
	...
	price.Mul(price, big.NewInt(int64(self.pbf.GpobaseCorrectionFactor)))
	price.Div(price, big.NewInt(100))
	if price.Cmp(self.minPrice) < 0 {
		price.Set(self.minPrice)
	} else if self.pbf.GpoMaxGasPrice != nil && price.Cmp(self.pbf.GpoMaxGasPrice) > 0 {
		price.Set(self.pbf.GpoMaxGasPrice)
	}
	return price
}
