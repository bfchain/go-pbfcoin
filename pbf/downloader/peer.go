...
// Hash and block fetchers belonging to pbf/61 and below
type relativeHashFetcherFn func(common.Hash) error
type absoluteHashFetcherFn func(uint64, int) error
type blockFetcherFn func([]common.Hash) error

// Block header and body fetchers belonging to pbf/62 and above
...
type peer struct {
	...
	statpbfroughput   float64 // Number of node data pieces measured to be retrievable per second
	...
	getRelHashes relativeHashFetcherFn // [pbf/61] method to retrieve a batch of hashes from an origin hash
	getAbsHashes absoluteHashFetcherFn // [pbf/61] method to retrieve a batch of hashes from an absolute position
	getBlocks    blockFetcherFn        // [pbf/61] method to retrieve a batch of blocks

	getRelHeaders  relativeHeaderFetcherFn // [pbf/62] method to retrieve a batch of headers from an origin hash
	getAbsHeaders  absoluteHeaderFetcherFn // [pbf/62] method to retrieve a batch of headers from an absolute position
	getBlockBodies blockBodyFetcherFn      // [pbf/62] method to retrieve a batch of block bodies

	getReceipts receiptFetcherFn // [pbf/63] method to retrieve a batch of block transaction receipts
	getNodeData stateFetcherFn   // [pbf/63] method to retrieve a batch of state trie data

	version int // pbf protocol version number to switch strategies
	lock    sync.RWMutex
}
...
func (p *peer) Fetch61(request *fetchRequest) error {
	...
	if p.version != 61 {
		panic(fmt.Sprintf("block fetch [pbf/61] requested on pbf/%d", p.version))
	}
...
func (p *peer) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [pbf/62+] requested on pbf/%d", p.version))
	}
	// Short circuit if the peer is already fetching
	if !atomic.CompareAndSwapInt32(&p.blockIdle, 0, 1) {
		return errAlreadyFetching
...
func (p *peer) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [pbf/63+] requested on pbf/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchNodeData(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [pbf/63+] requested on pbf/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) SetNodeDataIdle(delivered int) {
	p.setIdle(p.stateStarted, delivered, &p.statpbfroughput, &p.stateIdle)
}
...
func (p *peer) NodeDataCapacity() int {
	...
	return int(math.Max(1, math.Min(p.statpbfroughput*float64(stateTargetRTT)/float64(time.Second), float64(MaxStateFetch))))
}
...
// Lacks retrieves whpbfer the hash of a blockchain item is on the peers lacking
// list (i.e. whpbfer we know that the peer does not have it).
...
func (p *peer) String() string {
	...
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("blocks %3.2f/s, ", p.blockThroughput)+
			...
			fmt.Sprintf("states %3.2f/s, ", p.statpbfroughput)+
...
func (ps *peerSet) Register(p *peer) error {
	...
	if len(ps.peers) > 0 {
		p.blockThroughput, p.receiptThroughput, p.statpbfroughput = 0, 0, 0

		for _, peer := range ps.peers {
			peer.lock.RLock()
			p.blockThroughput += peer.blockThroughput
			p.receiptThroughput += peer.receiptThroughput
			p.statpbfroughput += peer.statpbfroughput
			peer.lock.RUnlock()
		}
		p.blockThroughput /= float64(len(ps.peers))
		p.receiptThroughput /= float64(len(ps.peers))
		p.statpbfroughput /= float64(len(ps.peers))
	}
	ps.peers[p.id] = p
	return nil
}
