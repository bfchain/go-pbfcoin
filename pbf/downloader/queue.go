...
import (
	...
	"github.com/pbfcoin/go-pbfcoin/trie"
...
type fetchRequest struct {
	...
	Hashes  map[common.Hash]int // [pbf/61] Requested hashes with their insertion index (priority)
	Headers []*types.Header     // [pbf/62] Requested headers, sorted by request order
...
type queue struct {
	...
	hashPool    map[common.Hash]int // [pbf/61] Pending hashes, mapping to their insertion index (priority)
	hashQueue   *prque.Prque        // [pbf/61] Priority queue of the block hashes to fetch
	hashCounter int                 // [pbf/61] Counter indexing the added hashes to ensure retrieval order

	headerHead common.Hash // [pbf/62] Hash of the last queued header to verify order

	blockTaskPool  map[common.Hash]*types.Header // [pbf/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [pbf/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [pbf/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [pbf/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [pbf/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [pbf/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [pbf/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [pbf/63] Set of the completed receipt fetches

	stateTaskIndex int                      // [pbf/63] Counter indexing the added hashes to ensure prioritised retrieval order
	stateTaskPool  map[common.Hash]int      // [pbf/63] Pending node data retrieval tasks, mapping to their priority
	stateTaskQueue *prque.Prque             // [pbf/63] Priority queue of the hashes to fetch the node data for
	statePendPool  map[string]*fetchRequest // [pbf/63] Currently pending node data retrieval operations

	stateDatabase   pbfdb.Database   // [pbf/63] Trie database to populate during state reassembly
	stateScheduler  *state.StateSync // [pbf/63] State trie synchronisation scheduler and integrator
	stateProcessors int32            // [pbf/63] Number of currently running state processors
	stateSchedLock  sync.RWMutex     // [pbf/63] Lock serialising access to the state scheduler
...
func newQueue(stateDb pbfdb.Database) *queue {
	lock := new(sync.Mutex)
	return &queue{
...
// InFlightBlocks retrieves whpbfer there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whpbfer there are receipt fetch requests currently
// in flight.
...
// InFlightNodeData retrieves whpbfer there are node data entry fetch requests
// currently in flight.
...
func (q *queue) reserveHashes(p *peer, count int, taskQueue *prque.Prque, taskGen func(int), pendPool map[string]*fetchRequest, maxPending int) *fetchRequest {
	// Short circuit if the peer's already downloading sompbfing (sanity check to
	// not corrupt state)
...
// returns a flag whpbfer empty blocks were queued requiring processing.
...
// also returns a flag whpbfer empty receipts were queued requiring importing.
...
func (q *queue) reserveHeaders(p *peer, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	// downloading sompbfing (sanity check not to corrupt state)
	if taskQueue.Empty() {
