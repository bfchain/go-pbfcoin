...
var (
	...
	hashTTL        = 3 * time.Second     // [pbf/61] Time it takes for a hash request to time out
	blockTargetRTT = 3 * time.Second / 2 // [pbf/61] Target time for completing a block retrieval request
	blockTTL       = 3 * blockTargetRTT  // [pbf/61] Maximum time allowance before a block request is considered expired

	headerTTL        = 3 * time.Second      // [pbf/62] Time it takes for a header request to time out
	bodyTargetRTT    = 3 * time.Second / 2  // [pbf/62] Target time for completing a block body retrieval request
	bodyTTL          = 3 * bodyTargetRTT    // [pbf/62] Maximum time allowance before a block body request is considered expired
	receiptTargetRTT = 3 * time.Second / 2  // [pbf/63] Target time for completing a receipt retrieval request
	receiptTTL       = 3 * receiptTargetRTT // [pbf/63] Maximum time allowance before a receipt request is considered expired
	stateTargetRTT   = 2 * time.Second / 2  // [pbf/63] Target time for completing a state trie retrieval request
	stateTTL         = 3 * stateTargetRTT   // [pbf/63] Maximum time allowance before a node data request is considered expired

	maxQueuedHashes   = 256 * 1024 // [pbf/61] Maximum number of hashes to queue for import (DOS protection)
	maxQueuedHeaders  = 256 * 1024 // [pbf/62] Maximum number of headers to queue for import (DOS protection)
	maxQueuedStates   = 256 * 1024 // [pbf/63] Maximum number of state requests to queue (DOS protection)
...
type Downloader struct {
	...
	gpbfeader        headerRetrievalFn        // Retrieves a header from the chain
	...
	// Channels
	newPeerCh     chan *peer
	hashCh        chan dataPack // [pbf/61] Channel receiving inbound hashes
	blockCh       chan dataPack // [pbf/61] Channel receiving inbound blocks
	headerCh      chan dataPack // [pbf/62] Channel receiving inbound block headers
	bodyCh        chan dataPack // [pbf/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack // [pbf/63] Channel receiving inbound receipts
	stateCh       chan dataPack // [pbf/63] Channel receiving inbound node state data
	blockWakeCh   chan bool     // [pbf/61] Channel to signal the block fetcher of new tasks
	bodyWakeCh    chan bool     // [pbf/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool     // [pbf/63] Channel to signal the receipt fetcher of new tasks
	stateWakeCh   chan bool     // [pbf/63] Channel to signal the state fetcher of new tasks
...
func New(stateDb pbfdb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
	gpbfeader headerRetrievalFn, getBlock blockRetrievalFn, headHeader headHeaderRetrievalFn, headBlock headBlockRetrievalFn,
...
// Synchronising returns whpbfer the downloader is currently retrieving blocks.
...
func (d *Downloader) RegisterPeer(id string, version int, head common.Hash,
	getRelHashes relativeHashFetcherFn, getAbsHashes absoluteHashFetcherFn, getBlocks blockFetcherFn, // pbf/61 callbacks, remove when upgrading
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	glog.V(logger.Debug).Infof("Synchronising with the network using: %s [pbf/%d]", p.id, p.version)
	...
	default:
		// Sompbfing very wrong, stop right here
		glog.V(logger.Error).Infof("Unsupported pbf protocol: %d", p.version)
		return errBadPeer
	}
}
...
func (d *Downloader) fetchHeight61(p *peer) (uint64, error) {
	...
	for {
		...
		case packet := <-d.blockCh:
			...
			// Make sure the peer actually gave sompbfing valid
		...
		case <-d.stateCh:
		case <-d.receiptCh:
			// Ignore pbf/{62,63} packets because this is pbf/61.
	...
	for {
		...
		case packet := <-d.hashCh:
			...
			if packet.Items() == 0 {
				...
				// L: Request new hashes up from 11 (R's TD was higher, it must have sompbfing)
				// R: Nothing to give
	...
	for {
		...
		case packet := <-d.blockCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered sompbfing completely else than requested (usually
...
func (d *Downloader) fetchHeight(p *peer) (uint64, error) {
	...
	for {
		...
		case <-d.hashCh:
		case <-d.blockCh:
			// Ignore pbf/61 packets because this is pbf/62+.
			// These can arrive as a late delivery from a previous sync.
		}
	}
}
	...
	for start+1 < end {
		...
		for arrived := false; !arrived; {
			...
			case packer := <-d.headerCh:
				...
				header := d.gpbfeader(headers[0].Hash()) // Independent of sync mode, header surely exists
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if packet.Items() == 0 {
				...
				// L: Import of block 11 finishes
				// L: Sync begins, and finds common ancestor at 11
				// L: Request new headers up from 11 (R's TD was higher, it must have sompbfing)
				// R: Nothing to give
				...
				// peer gave us sompbfing useful, we're already happy/progressed (above check).
