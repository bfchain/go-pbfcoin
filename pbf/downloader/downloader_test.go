...
func newTester() *downloadTester {
	...
	tester.stateDb, _ = pbfdb.NewMemDatabase()
	tester.stateDb.Put(genesis.Root().Bytes(), []byte{0x00})

	tester.downloader = New(tester.stateDb, new(event.TypeMux), tester.hasHeader, tester.hasBlock, tester.gpbfeader,
...
// gpbfeader retrieves a header from the testers canonical chain.
func (dl *downloadTester) gpbfeader(hash common.Hash) *types.Header {
	dl.lock.RLock()
...
// peerGetRelHashesFn constructs a Gpbfashes function associated with a specific
...
// peerGetAbsHashesFn constructs a GpbfashesFromNumber function associated with
