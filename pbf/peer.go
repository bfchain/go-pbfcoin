...
// PeerInfo represents a short summary of the pbfcoin sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // pbfcoin protocol version negotiated
...
// Spbfead updates the head (most recent) hash of the peer.
func (p *peer) Spbfead(hash common.Hash) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
// Handshake executes the pbf protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("pbf/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the pbfcoin sub-protocol.
type peerSet struct {
