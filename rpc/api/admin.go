...
var (
	...
	AdminMapping = map[string]adminhandler{
		...
		"admin_spbfashReg":         (*adminApi).SpbfashReg,
...
func NewAdminApi(xpbf *xpbf.Xpbf, pbfcoin *pbf.pbfcoin, codec codec.Codec) *adminApi {
	return &adminApi{
		xpbf:    xpbf,
		pbfcoin: pbfcoin,
		codec:   codec,
...
func (self *adminApi) AddPeer(req *shared.Request) (interface{}, error) {
	...
	err := self.pbfcoin.AddPeer(args.Url)
	if err == nil {
		return true, nil
	}
	return false, err
...
func (self *adminApi) Peers(req *shared.Request) (interface{}, error) {
	return self.pbfcoin.Network().PeersInfo(), nil
}

func (self *adminApi) NodeInfo(req *shared.Request) (interface{}, error) {
	return self.pbfcoin.Network().NodeInfo(), nil
}

func (self *adminApi) DataDir(req *shared.Request) (interface{}, error) {
	return self.pbfcoin.DataDir, nil
}
...
func (self *adminApi) ImportChain(req *shared.Request) (interface{}, error) {
	...
	for batch := 0; ; batch++ {
		...
		// Import the batch.
		if hasAllBlocks(self.pbfcoin.BlockChain(), blocks[:i]) {
			continue
		}
		if _, err := self.pbfcoin.BlockChain().InsertChain(blocks[:i]); err != nil {
...
func (self *adminApi) ExportChain(req *shared.Request) (interface{}, error) {
	...
	defer fh.Close()
	if err := self.pbfcoin.BlockChain().Export(fh); err != nil {
		return false, err
	}
...
func (self *adminApi) SetSolc(req *shared.Request) (interface{}, error) {
	...
	solc, err := self.xpbf.SetSolc(args.Path)
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) StartRPC(req *shared.Request) (interface{}, error) {
	...
	apis, err := ParseApiString(args.Apis, self.codec, self.xpbf, self.pbfcoin)
	if err != nil {
		return false, err
	}
...
func (self *adminApi) SleepBlocks(req *shared.Request) (interface{}, error) {
	...
	height = new(big.Int).Add(self.xpbf.CurrentBlock().Number(), big.NewInt(args.N))
	height, err = sleepBlocks(self.xpbf.UpdateState(), height, timer)
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) SpbfashReg(req *shared.Request) (interface{}, error) {
	args := new(SpbfashRegArgs)
	if err := self.coder.Decode(req.Params, &args); err != nil {
		return nil, shared.NewDecodeParamError(err.Error())
	}
	...
	txhash, err := reg.SpbfashReg(args.HashReg, sender)
	if err != nil {
		return false, err
	}
...
func (self *adminApi) Register(req *shared.Request) (interface{}, error) {
	...
	codeb := self.xpbf.CodeAtBytes(args.Address)
	codeHash := common.BytesToHash(crypto.Sha3(codeb))
	contentHash := common.HexToHash(args.ContentHashHex)
	registry := registrar.New(self.xpbf)

	_, err := registry.SpbfashToHash(sender, codeHash, contentHash)
	if err != nil {
		return false, err
	}
...
func (self *adminApi) GetContractInfo(req *shared.Request) (interface{}, error) {
	...
	infoDoc, err := natspec.FetchDocsForContract(args.Contract, self.xpbf, self.pbfcoin.HTTPClient())
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) HttpGet(req *shared.Request) (interface{}, error) {
	...
	resp, err := self.pbfcoin.HTTPClient().Get(args.Uri, args.Path)
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) EnableUserAgent(req *shared.Request) (interface{}, error) {
	if fe, ok := self.xpbf.Frontend().(*useragent.RemoteFrontend); ok {
		fe.Enable()
	}
	return true, nil
}
