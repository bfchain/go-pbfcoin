...
type SetpbferbaseArgs struct {
	pbferbase common.Address
}

func (args *SetpbferbaseArgs) UnmarshalJSON(b []byte) (err error) {
	var obj []interface{}
	if err := json.Unmarshal(b, &obj); err != nil {
		return shared.NewDecodeParamError(err.Error())
	}
	...
	if addr, ok := obj[0].(string); ok {
		args.pbferbase = common.HexToAddress(addr)
		if (args.pbferbase == common.Address{}) {
			return shared.NewInvalidTypeError("pbferbase", "not a valid address")
		}
		return nil
	}

	return shared.NewInvalidTypeError("pbferbase", "not a string")
}
