...
func TestParseApiString(t *testing.T) {
	...
	apis, err = ParseApiString("pbf", codec.JSON, nil, nil)
	if err != nil {
	...
	apis, err = ParseApiString("pbf,pbf", codec.JSON, nil, nil)
	if err != nil {
		t.Errorf("Expected nil err from parsing empty API string but got \"%v\"", err)
	}
	...
	apis, err = ParseApiString("pbf,invalid", codec.JSON, nil, nil)
	if err == nil {
...
func TestCompileSolidity(t *testing.T) {
	...
	pbf := &pbf.pbfcoin{}
	xpbf := xpbf.NewTest(pbf, nil)
	api := NewethApi(xpbf, pbf, codec.JSON)
