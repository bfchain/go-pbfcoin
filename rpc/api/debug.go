...
var (
	...
	DebugMapping = map[string]debughandler{
		...
		"debug_spbfead":      (*debugApi).Spbfead,
...
func NewDebugApi(xpbf *xpbf.Xpbf, pbfcoin *pbf.pbfcoin, coder codec.Codec) *debugApi {
	return &debugApi{
		xpbf:    xpbf,
		pbfcoin: pbfcoin,
...
func (self *debugApi) DumpBlock(req *shared.Request) (interface{}, error) {
	...
	stateDb, err := state.New(block.Root(), self.pbfcoin.ChainDb())
	if err != nil {
		return nil, err
	}
...
func (self *debugApi) Spbfead(req *shared.Request) (interface{}, error) {
	args := new(BlockNumArg)
	if err := self.codec.Decode(req.Params, &args); err != nil {
		return nil, shared.NewDecodeParamError(err.Error())
	}
	self.pbfcoin.BlockChain().Spbfead(uint64(args.BlockNumber))
...
func (self *debugApi) ProcessBlock(req *shared.Request) (interface{}, error) {
	...
	var (
		blockchain = self.pbfcoin.BlockChain()
		validator  = blockchain.Validator()
		processor  = blockchain.Processor()
	)

	err := core.ValidateHeader(blockchain.AuxValidator(), block.Header(), blockchain.Gpbfeader(block.ParentHash()), true, false)
	if err != nil {
		return false, err
	}
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), self.pbfcoin.ChainDb())
	if err != nil {
		return false, err
	}
...
func (self *debugApi) SeedHash(req *shared.Request) (interface{}, error) {
	...
	if hash, err := pbfash.GetSeedHash(uint64(args.BlockNumber)); err == nil {
