...
var (
	...
	DefaultHttpRpcApis = strings.Join([]string{
		shared.DbApiName, shared.pbfApiName, shared.NetApiName, shared.Web3ApiName,
	}, ",")
)

type pbfcoinClient interface {
...
func handle(id int, conn net.Conn, api shared.pbfcoinApi, c codec.Codec) {
	codec := c.New(conn)
...
// ${protocol}:${path}
// e.g. ipc:/tmp/gpbf.ipc
//      rpc:localhost:8545
func ClientFromEndpoint(endpoint string, c codec.Codec) (pbfcoinClient, error) {
