...
/*
Registrar implements the pbfcoin name registrar services mapping
- arbitrary strings to pbfcoin addresses
- hashes to hashes
...
// Registrar's backend is defined as an interface (implemented by xpbf, but could be remote)
...
func (self *Registrar) SpbfashReg(hashreg string, addr common.Address) (txhash string, err error) {
	if hashreg != "" {
		HashRegAddr = hashreg
	} else {
...
func (self *Registrar) SpbfashToHash(address common.Address, codehash, dochash common.Hash) (txh string, err error) {
	if zero.MatchString(HashRegAddr) {
		return "", fmt.Errorf("HashReg address is not set")
	}
	...
	glog.V(logger.Detail).Infof("SpbfashToHash data: %s sent  to %v\n", data, HashRegAddr)
	return self.backend.Transact(
		address.Hex(),
		HashRegAddr,
		"", "", "", "",
