...
func GetNotice(xpbf *xpbf.Xpbf, tx string, http *httpclient.HTTPClient) (notice string) {
	ns, err := New(xpbf, tx, http)
	if err != nil {
		if ns == nil {
...
func New(xpbf *xpbf.Xpbf, jsontx string, http *httpclient.HTTPClient) (self *NatSpec, err error) {
	...
	content, err := FetchDocsForContract(contractAddress, xpbf, http)
	if err != nil {
		return
	}
...
func FetchDocsForContract(contractAddress string, xpbf *xpbf.Xpbf, client *httpclient.HTTPClient) (content []byte, err error) {
	// retrieve contract hash from state
	codehex := xpbf.CodeAt(contractAddress)
	codeb := xpbf.CodeAtBytes(contractAddress)
...
func (self *NatSpec) makeAbi2method(abiKey [8]byte) (mpbf *method) {
...
func (self *NatSpec) Notice() (notice string, err error) {
	...
	mpbf := self.makeAbi2method(abiKey)

	if mpbf == nil {
		err = fmt.Errorf("abi key does not match any method")
		return
	}
	notice, err = self.noticeFormethod(self.tx, mpbf.name, mpbf.Notice)
	return
}
