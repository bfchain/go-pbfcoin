...
import (
	...
	xe "github.com/pbfcoin/go-pbfcoin/xpbf"
)
...
func (self *testFrontend) UnlockAccount(acc []byte) bool {
	self.pbfcoin.AccountManager().Unlock(common.BytesToAddress(acc), "password")
	return true
}
...
func (self *testFrontend) ConfirmTransaction(tx string) bool {
	if self.wantNatSpec {
		...
		self.lastConfirm = GetNotice(self.xpbf, tx, client)
	}
	return true
}

func testpbf(t *testing.T) (pbfcoin *pbf.pbfcoin, err error) {
	...
	return pbf.New(&pbf.Config{
		DataDir:                 tmp,
		AccountManager:          am,
		pbferbase:               common.HexToAddress(testAddress),
		MaxPeers:                0,
		PowTest:                 true,
...
func testInit(t *testing.T) (self *testFrontend) {
	// initialise and start minimal pbfcoin stack
	pbfcoin, err := testpbf(t)
	if err != nil {
		t.Errorf("error creating pbfcoin: %v", err)
		return
	}
	err = pbfcoin.Start()
	if err != nil {
	...
	// mock frontend
	self = &testFrontend{t: t, pbfcoin: pbfcoin}
	self.xpbf = xe.New(pbfcoin, self)
	self.wait = self.xpbf.UpdateState()
	addr, _ := self.pbfcoin.pbferbase()
	...
	recH := self.xpbf.GetTxReceipt(common.HexToHash(txH))
	if recH == nil {
	...
	recU := self.xpbf.GetTxReceipt(common.HexToHash(txU))
	if recU == nil {
...
func TestNatspecE2E(t *testing.T) {
	...
	tf := testInit(t)
	defer tf.pbfcoin.Stop()
	addr, _ := tf.pbfcoin.pbferbase()
	...
	codeb := tf.xpbf.CodeAtBytes(registrar.HashRegAddr)
	codehash := crypto.Sha3Hash(codeb)

	reg := registrar.New(tf.xpbf)
	_, err := reg.SpbfashToHash(addr, codehash, dochash)
	if err != nil {
