...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `Gpbf Monitor: node metrics monitoring and visualization`,
		Description: `
The Gpbf monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) {
	...
	// Attach to an pbfcoin node over IPC or RPC
	...
	xpbf := rpc.NewXpbf(client)

	// Retrieve all the available metrics and resolve the user pattens
	metrics, err := retrieveMetrics(xpbf)
	if err != nil {
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gpbf (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
	...
	refreshCharts(xpbf, monitored, data, units, charts, ctx, footer)
	termui.Body.Align()
	...
	for {
		...
		case <-refresh:
			if refreshCharts(xpbf, monitored, data, units, charts, ctx, footer) {
				termui.Body.Align()
			}
			termui.Render(termui.Body)
		}
...
// retrieveMetrics contacts the attached gpbf node and retrieves the entire set
// of collected system metrics.
func retrieveMetrics(xpbf *rpc.Xpbf) (map[string]interface{}, error) {
	return xpbf.Call("debug_metrics", []interface{}{true})
}
...
func refreshCharts(xpbf *rpc.Xpbf, metrics []string, data [][]float64, units []int, charts []*termui.LineChart, ctx *cli.Context, footer *termui.Par) (realign bool) {
	values, err := retrieveMetrics(xpbf)
