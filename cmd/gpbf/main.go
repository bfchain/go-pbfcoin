...
func init() {
	...
	app = utils.NewApp(Version, "the go-pbfcoin command line interface")
	app.Action = run
...
`,
		...
		monitorCommand,
		{
			Action: makedag,
			Name:   "makedag",
			Usage:  "generate pbfash dag (for testing)",
			Description: `
The makedag command generates an pbfash DAG in /tmp/dag.
...
`,
		},
		{
			Action: version,
			Name:   "version",
			Usage:  "print pbfcoin version numbers",
			Description: `
...
`,
		...
		{
			Name:  "wallet",
			Usage: "pbfcoin presale wallet",
			Subcommands: []cli.Command{
				{
					Action: importWallet,
					Name:   "import",
					Usage:  "import pbfcoin presale wallet",
				},
			},
			Description: `
...
will prompt for your password and imports your pbfer presale account.
...
between pbfcoin nodes by simply copying.
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    pbfcoin --password <passwordfile> account new
...
password to file or expose in any other way.
    ...
    pbfcoin account update <address>
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    pbfcoin --password <passwordfile> account update <address>
...
changes.
    ...
    pbfcoin account import <keyfile>
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    pbfcoin --password <passwordfile> account import <keyfile>

Note:
As you can directly copy your encrypted accounts to another pbfcoin instance,
...
nodes.
		...
		},
		{
			Action: console,
			Name:   "console",
			Usage:  `Gpbf Console: interactive JavaScript environment`,
			Description: `
The Gpbf console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/pbfcoin/go-pbfcoin/wiki/Javascipt-Console
`},
		{
			Action: attach,
			Name:   "attach",
			Usage:  `Gpbf Console: interactive JavaScript environment (connect to node)`,
			Description: `
The Gpbf console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/pbfcoin/go-pbfcoin/wiki/Javascipt-Console.
This command allows to open a console on a running gpbf node.
`,
		},
		{
			Action: execJSFiles,
			Name:   "js",
			Usage:  `executes the given JavaScript files in the Gpbf JavaScript VM`,
			Description: `
The JavaScript VM exposes a node admin interface as well as the Ðapp
JavaScript API. See https://github.com/pbfcoin/go-pbfcoin/wiki/Javascipt-Console
`,
		},
	}
	app.Flags = []cli.Flag{
...
func run(ctx *cli.Context) {
	cfg := utils.MakepbfConfig(ClientIdentifier, nodeNameVersion, ctx)
...
func attach(ctx *cli.Context) {
	var client comms.pbfcoinClient
	var err error
	if ctx.Args().Present() {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to gpbf node - %v", err)
	}
...
func blockRecovery(ctx *cli.Context) {
	...
	cfg := utils.MakepbfConfig(ClientIdentifier, nodeNameVersion, ctx)
	blockDb, err := pbfdb.NewLDBDatabase(filepath.Join(cfg.DataDir, "blockchain"), cfg.DatabaseCache)
	if err != nil {
...
func startpbf(ctx *cli.Context, pbf *pbf.pbfcoin) {
	// Start pbfcoin itself
	utils.Startpbfcoin(pbf)

	am := pbf.AccountManager()
	...
	if !ctx.GlobalBool(utils.IPCDisabledFlag.Name) {
		if err := utils.StartIPC(pbf, ctx); err != nil {
			utils.Fatalf("Error string IPC: %v", err)
		}
	}
	if ctx.GlobalBool(utils.RPCEnabledFlag.Name) {
		if err := utils.StartRPC(pbf, ctx); err != nil {
...
func makedag(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gpbf makedag <block number> <outputdir>`)
	}
	switch {
	case len(args) == 2:
...
func gpuinfo(ctx *cli.Context) {
	pbf.PrintOpenCLDevices()
}

func gpubench(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gpbf gpubench <gpu number>`)
	}
	switch {
	case len(args) == 1:
...
func version(c *cli.Context) {
	...
	fmt.Println("Protocol Versions:", pbf.ProtocolVersions)
