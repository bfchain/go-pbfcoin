...
type testjpbfre struct {
	*jsre
	lastConfirm string
	client      *httpclient.HTTPClient
}

func (self *testjpbfre) UnlockAccount(acc []byte) bool {
	err := self.pbfcoin.AccountManager().Unlock(common.BytesToAddress(acc), "")
	if err != nil {
...
func (self *testjpbfre) ConfirmTransaction(tx string) bool {
	if self.pbfcoin.NatSpec {
		self.lastConfirm = natspec.GetNotice(self.xpbf, tx, self.client)
	}
	return true
}

func testJpbfRE(t *testing.T) (string, *testjpbfre, *pbf.pbfcoin) {
	return testREPL(t, nil)
}

func testREPL(t *testing.T, config func(*pbf.Config)) (string, *testjpbfre, *pbf.pbfcoin) {
	tmp, err := ioutil.TempDir("", "gpbf-test")
	if err != nil {
		t.Fatal(err)
	}
	...
	conf := &pbf.Config{
		...
		PowTest:        true,
		NewDB:          func(path string) (pbfdb.Database, error) { return db, nil },
	}
	if config != nil {
		config(conf)
	}
	pbfcoin, err := pbf.New(conf)
	if err != nil {
		t.Fatal("%v", err)
	}
	...
	assetPath := filepath.Join(os.Getenv("GOPATH"), "src", "github.com", "pbfcoin", "go-pbfcoin", "cmd", "mist", "assets", "ext")
	client := comms.NewInProcClient(codec.JSON)
	tf := &testjpbfre{client: pbfcoin.HTTPClient()}
	repl := newJSRE(pbfcoin, assetPath, "", client, false, tf)
	tf.jsre = repl
	return tmp, tf, pbfcoin
}

func TestNodeInfo(t *testing.T) {
	t.Skip("broken after p2p update")
	tmp, repl, pbfcoin := testJpbfRE(t)
	if err := pbfcoin.Start(); err != nil {
		t.Fatalf("error starting pbfcoin: %v", err)
	}
	defer pbfcoin.Stop()
...
func TestAccounts(t *testing.T) {
	...
	checkEvalJSON(t, repl, `pbf.accounts`, `["`+testAddress+`"]`)
	checkEvalJSON(t, repl, `pbf.coinbase`, `"`+testAddress+`"`)
	...
	checkEvalJSON(t, repl, `pbf.accounts`, `["`+testAddress+`","`+addr+`"]`)
...
func TestBlockChain(t *testing.T) {
	...
	val, err := repl.re.Run("JSON.stringify(debug.dumpBlock(pbf.blockNumber))")
	if err != nil {
		t.Errorf("expected no error, got %v", err)
	}
	...
	// do the export
	extmp, err := ioutil.TempDir("", "gpbf-test-export")
	if err != nil {
		t.Fatal(err)
	}
	...
	checkEvalJSON(t, repl, `debug.dumpBlock(pbf.blockNumber)`, beforeExport)
}
...
func TestMining(t *testing.T) {
	...
	defer pbfcoin.Stop()
	defer os.RemoveAll(tmp)
	checkEvalJSON(t, repl, `pbf.mining`, `false`)
}

func TestRPC(t *testing.T) {
	tmp, repl, pbfcoin := testJpbfRE(t)
	if err := pbfcoin.Start(); err != nil {
		t.Errorf("error starting pbfcoin: %v", err)
		return
	}
	defer pbfcoin.Stop()
	defer os.RemoveAll(tmp)

	checkEvalJSON(t, repl, `admin.startRPC("127.0.0.1", 5004, "*", "web3,pbf,net")`, `true`)
}
...
func TestCheckTestAccountBalance(t *testing.T) {
	...
	checkEvalJSON(t, repl, `pbf.getBalance(primary)`, `"`+testBalance+`"`)
}
...
func TestSignature(t *testing.T) {
	...
	val, err := repl.re.Run(`pbf.sign("` + testAddress + `", "` + testHash + `")`)
...
func TestContract(t *testing.T) {
	t.Skip("contract testing is implemented with mining in pbfash test mode. This takes about 7seconds to run. Unskip and run on demand")
	coinbase := common.HexToAddress(testAddress)
	tmp, repl, pbfcoin := testREPL(t, func(conf *pbf.Config) {
		conf.pbferbase = coinbase
		conf.PowTest = true
	})
	if err := pbfcoin.Start(); err != nil {
		t.Errorf("error starting pbfcoin: %v", err)
	...
	reg := registrar.New(repl.xpbf)
	_, err := reg.SetGlobalRegistrar("", coinbase)
	if err != nil {
		t.Errorf("error setting HashReg: %v", err)
	}
	_, err = reg.SpbfashReg("", coinbase)
	if err != nil {
		t.Errorf("error setting HashReg: %v", err)
	}
	...
	* then set once more SpbfashReg SetUrlHint
	 */
	...
	if checkEvalJSON(t, repl, `primary = pbf.accounts[0]`, `"`+testAddress+`"`) != nil {
		return
	}
	...
	if err != nil {
		...
		if checkEvalJSON(t, repl, `contract = pbf.compile.solidity(source).test`, string(contractInfo)) != nil {
			return
		}
	}
	...
	if checkEvalJSON(
		t, repl,
		`contractaddress = pbf.sendTransaction({from: primary, data: contract.code})`,
...
func pendingTransactions(repl *testjpbfre, t *testing.T) (txc int64, err error) {
	txs := repl.pbfcoin.TxPool().GetTransactions()
	return int64(len(txs)), nil
}

func processTxs(repl *testjpbfre, t *testing.T, expTxc int) bool {
	var txc int64
	var err error
	for i := 0; i < 50; i++ {
	...
	err = repl.pbfcoin.StartMining(runtime.NumCPU(), "")
	if err != nil {
		t.Errorf("unexpected error mining: %v", err)
		return false
	}
	defer repl.pbfcoin.StopMining()

	timer := time.NewTimer(100 * time.Second)
	height := new(big.Int).Add(repl.xpbf.CurrentBlock().Number(), big.NewInt(1))
	repl.wait <- height
	select {
	case <-timer.C:
		// if times out make sure the xpbf loop does not block
		go func() {
			select {
			case repl.wait <- nil:
			case <-repl.wait:
...
func checkEvalJSON(t *testing.T, re *testjpbfre, expr, want string) error {
