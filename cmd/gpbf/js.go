...
import (
	...
	re "github.com/pbfcoin/go-pbfcoin/jsre"
	"github.com/pbfcoin/go-pbfcoin/pbf"
	"github.com/pbfcoin/go-pbfcoin/rpc"
	"github.com/pbfcoin/go-pbfcoin/rpc/api"
...
func newLightweightJSRE(docRoot string, client comms.pbfcoinClient, datadir string, interactive bool) *jsre {
	js := &jsre{ps1: "> "}
...
func newJSRE(pbfcoin *pbf.pbfcoin, docRoot, corsDomain string, client comms.pbfcoinClient, interactive bool, f xpbf.Frontend) *jsre {
	js := &jsre{pbfcoin: pbfcoin, ps1: "> "}
	// set default cors domain used by startRpc from CLI flag
	js.corsDomain = corsDomain
	if f == nil {
		f = js
	}
	js.xpbf = xpbf.New(pbfcoin, f)
	js.wait = js.xpbf.UpdateState()
	js.client = client
	if clt, ok := js.client.(*comms.InProcClient); ok {
		if offeredApis, err := api.ParseApiString(shared.AllApis, codec.JSON, js.xpbf, pbfcoin); err == nil {
	...
	if !liner.TerminalSupported() || !interactive {
		...
		lr := liner.NewLiner()
		js.withHistory(pbfcoin.DataDir, func(hist *os.File) { lr.ReadHistory(hist) })
		lr.SetCtrlCAborts(true)
		js.loadAutoCompletion()
		lr.SetWordCompleter(apiWordCompleter)
		lr.SetTabCompletionStyle(liner.TabPrints)
		js.prompter = lr
		js.atexit = func() {
			js.withHistory(pbfcoin.DataDir, func(hist *os.File) { hist.Truncate(0); lr.WriteHistory(hist) })
			lr.Close()
			close(js.wait)
		}
	}
...
// show summary of current gpbf instance
func (self *jsre) welcome() {
	self.re.Run(`
    (function () {
      console.log('instance: ' + web3.version.client);
      console.log(' datadir: ' + admin.datadir);
      console.log("coinbase: " + pbf.coinbase);
      var ts = 1000 * pbf.getBlock(pbf.blockNumber).timestamp;
      console.log("at block: " + pbf.blockNumber + " (" + new Date(ts) + ")");
    })();
  `)
	if modules, err := self.supportedApis(); err == nil {
...
func (js *jsre) apiBindings(f xpbf.Frontend) error {
	...
	apiImpl, err := api.ParseApiString(strings.Join(apiNames, ","), codec.JSON, js.xpbf, js.pbfcoin)
	if err != nil {
		utils.Fatalf("Unable to determine supported api's: %v", err)
	}

	jpbf := rpc.NewJpbf(api.Merge(apiImpl...), js.re, js.client, f)
	js.re.Set("jpbf", struct{}{})
	t, _ := js.re.Get("jpbf")
	jpbfObj := t.Object()

	jpbfObj.Set("send", jpbf.Send)
	jpbfObj.Set("sendAsync", jpbf.Send)
	...
	err = js.re.Compile("pbfcoin.js", re.Web3_JS)
	if err != nil {
	...
	_, err = js.re.Run("web3.setProvider(jpbf)")
	if err != nil {
		utils.Fatalf("Error setting web3 provider: %v", err)
	}

	// load only supported API's in javascript runtime
	shortcuts := "var pbf = web3.pbf; "
	...
	js.re.Run(`var GlobalRegistrar = pbf.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	return nil
}
...
func (self *jsre) ConfirmTransaction(tx string) bool {
	if self.pbfcoin.NatSpec {
		notice := natspec.GetNotice(self.xpbf, tx, self.pbfcoin.HTTPClient())
		fmt.Println(notice)
...
func (self *jsre) UnlockAccount(addr []byte) bool {
	...
	// TODO: allow retry
	if err := self.pbfcoin.AccountManager().Unlock(common.BytesToAddress(addr), pass); err != nil {
		return false
	} else {
