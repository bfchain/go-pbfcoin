...
// MakepbfConfig creates pbfcoin options from set command line flags.
func MakepbfConfig(clientID, version string, ctx *cli.Context) *pbf.Config {
	customName := ctx.GlobalString(IdentityFlag.Name)
	if len(customName) > 0 {
		clientID += "/" + customName
	}
	am := MakeAccountManager(ctx)
	pbferbase, err := ParamToAddress(ctx.GlobalString(pbferbaseFlag.Name), am)
	if err != nil {
		glog.V(logger.Error).Infoln("WARNING: No pbferbase set and no accounts found as default")
	}
	// Assemble the entire pbf configuration and return
	cfg := &pbf.Config{
		...
		pbferbase:               common.HexToAddress(pbferbase),
	...
	if ctx.GlobalBool(DevModeFlag.Name) {
		...
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			cfg.DataDir = os.TempDir() + "/pbfcoin_dev_mode"
		}
		cfg.PowTest = true
		cfg.DevMode = true
...
func MakeChain(ctx *cli.Context) (chain *core.BlockChain, chainDb pbfdb.Database) {
	datadir := MustDataDir(ctx)
	cache := ctx.GlobalInt(CacheFlag.Name)

	var err error
	if chainDb, err = pbfdb.NewLDBDatabase(filepath.Join(datadir, "chaindata"), cache); err != nil {
...
func IpcSocketPath(ctx *cli.Context) (ipcpath string) {
	if runtime.GOOS == "windows" {
		...
		ipcpath = common.DefaultIpcPath()
		if ctx.GlobalIsSet(DataDirFlag.Name) {
			ipcpath = filepath.Join(ctx.GlobalString(DataDirFlag.Name), "gpbf.ipc")
		}
		if ctx.GlobalIsSet(IPCPathFlag.Name) {
			ipcpath = ctx.GlobalString(IPCPathFlag.Name)
		}
...
func StartIPC(pbf *pbf.pbfcoin, ctx *cli.Context) error {
	config := comms.IpcConfig{
		Endpoint: IpcSocketPath(ctx),
	}

	initializer := func(conn net.Conn) (comms.Stopper, shared.pbfcoinApi, error) {
		fe := useragent.NewRemoteFrontend(conn, pbf.AccountManager())
		xpbf := xpbf.New(pbf, fe)
		apis, err := api.ParseApiString(ctx.GlobalString(IPCApiFlag.Name), codec.JSON, xpbf, pbf)
		if err != nil {
			return nil, nil, err
		}
		return xpbf, api.Merge(apis...), nil
	}

	return comms.StartIpc(config, codec.JSON, initializer)
}

func StartRPC(pbf *pbf.pbfcoin, ctx *cli.Context) error {
	...
	xpbf := xpbf.New(pbf, nil)
	codec := codec.JSON

	apis, err := api.ParseApiString(ctx.GlobalString(RpcApiFlag.Name), codec, xpbf, pbf)
	if err != nil {
		return err
	}
