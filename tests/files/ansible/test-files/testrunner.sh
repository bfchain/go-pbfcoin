...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-pbfcoin/build/test/createRandomStateTest\" pbfcoin/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pypbfcoin
	OUTPUT_PYTHON="$(docker run --rm pbfcoin/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm pbfcoin/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm pbfcoin/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
